...
<body>
		...
		<nav class="navbar navbar-expand-lg navbar-dark" id="navbar-especial">
			...
			<div class="collapse navbar-collapse" id="navbarNav">
				<ul class="navbar-nav">
					...
					<li class="nav-item"><a class="nav-link"
						href="/gerenciador/frotas">Frotas</a></li>
					<li class="nav-item active"><a class="nav-link" href="#">Carros<span
							class="sr-only">(current)</span></a></li>
					<li class="nav-item"><a class="nav-link "
						href="frota/carro/formCadastroCarros.jsp">Novo Carro</a></li>
				</ul>
			</div>
		</nav>
	</header>
	...
	<main class="container-fluid pt-3">
		<section class="container m-auto p-5" id="menu-carros">
			<section class="jumbotron jumbotron-fluid mb-2">
				<div class="container">
					<h1 display-4>Carros</h1>
					<p class="lead">central de registros</p>
				</div>
			</section>
			<!-- Colocar um Search e mostrar a lista -->
			<section id="pesquisa-carros">
				<nav class="navbar navbar-light bg-light">
					<form action="listaCarros" method="POST" class="form-inline"
						id="pesquisa-carro">
			...
			<section class="table-responsive" id="lista-carros">
				...
				<table class="table table-sm mt-3">
					...
					<tbody>
						<c:forEach items="${carros}" var="carro" varStatus="count">
							<tr>
								<th scope="row">${carro.id}</th>
								<td>${ carro.marcaVeiculo }</td>
								<td>${ carro.placaVeiculo }</td>
								<td>${ carro.portasCarro }</td>
								<td class="text-center">
									<div class="btn-group btn-group-sm" role="group"
										aria-label="botões em grupo">
										<a href="/gerenciador/exibeCarro?id=${carro.id}" type="button"
											class="btn btn-cor-especial">Editar</a> <a
											href="/gerenciador/removeCarro?id=${carro.id}" type="button"
											class="btn btn-dark">Remover</a>
									</div>
								</td>
							</tr>
						</c:forEach>
